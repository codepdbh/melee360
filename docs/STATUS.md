...
- M41 Particles and timed matches: `psDispParticles` is replaced by a native drawer that submits each particle as a textured view-space quad through the D3D HSD renderer, and the select phase adds timed matches (unlimited lives, KO credit from the last attacker, countdown and score HUD). Zelda/Sheik now spawn with a sleeping partner and transform through the original down special, and Popo fights with a CPU-driven Nana. Host link check only.
