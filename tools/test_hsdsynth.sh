...
if command -v cc >/dev/null 2>&1 && command -v c++ >/dev/null 2>&1; then
    ...
    c++ -std=c++17 -Wall -Wextra -Werror "${INC[@]}" \
        -c "$ROOT/src/xdk/hsdsynth_xdk.cpp" -o "$OBJ/hsdsynth_xdk.o"
    for f in synth devcom; do
        cc -std=c11 -Wall "${INC[@]}" \
            -include "$ROOT/src/xdk/hsdsynth_xdk_compat.h" \
            -c "$BASELIB/$f.c" -o "$OBJ/$f.o"
    done

    cc -std=c11 -Wall -Wextra -Werror -I"$BASELIB" "${INC[@]}" \
        -include "$ROOT/src/xdk/hsdsynth_xdk_compat.h" \
        -c "$ROOT/tests/host/test_hsdsynth.c" -o "$OBJ/test_hsdsynth.o"

    c++ "$OBJ/memory.o" "$OBJ/memory_xdk.o" "$OBJ/hsd_class_xdk.o" \
        "$OBJ/hash.o" "$OBJ/debug.o" "$OBJ/class.o" "$OBJ/object.o" \
        "$OBJ/objalloc.o" "$OBJ/id.o" "$OBJ/hsdsynth_xdk.o" \
        "$OBJ/synth.o" "$OBJ/devcom.o" \
        "$OBJ/test_hsdsynth.o" -lm -no-pie -o "$OUT"
    "$OUT"
    exit 0
fi
...
mkdir -p "$OBJ/msvc"
BAT="$OBJ/msvc/test_hsdsynth.bat"
...
cl %CC_NEW% /TP /EHsc /c %ROOT%\src\xdk\hsdsynth_xdk.cpp /Fo%OBJ%\hsdsynth_xdk.obj || exit /b 1
for %%f in (synth devcom) do (
    cl %CC_UP% /FI%ROOT%\src\xdk\hsdsynth_xdk_compat.h /c %BL%\%%f.c /Fo%OBJ%\%%f.obj || exit /b 1
)

cl %CC_NEW% /TC /I%BL% /FI%ROOT%\src\xdk\hsdsynth_xdk_compat.h /c %ROOT%\tests\host\test_hsdsynth.c /Fo%OBJ%\test_hsdsynth.obj || exit /b 1

link /nologo /LARGEADDRESSAWARE:NO /DYNAMICBASE:NO /OUT:%OBJ%\test_hsdsynth.exe %OBJ%\memory.obj %OBJ%\memory_xdk.obj %OBJ%\hsd_class_xdk.obj %OBJ%\hash.obj %OBJ%\debug.obj %OBJ%\class.obj %OBJ%\object.obj %OBJ%\objalloc.obj %OBJ%\id.obj %OBJ%\hsdsynth_xdk.obj %OBJ%\synth.obj %OBJ%\devcom.obj %OBJ%\test_hsdsynth.obj || exit /b 1
%OBJ%\test_hsdsynth.exe
exit /b %errorlevel%
BATCH
