...
if command -v cc >/dev/null 2>&1 && command -v c++ >/dev/null 2>&1; then
    ...
    cc -std=c11 -Wall -Wextra -Werror -I"$BASELIB" "${INC[@]}" \
        -include "$ROOT/src/xdk/hsdmath_xdk_compat.h" \
        -c "$ROOT/tests/host/test_hsdmath.c" -o "$OBJ/test_hsdmath.o"
    ...
    c++ "$OBJ/memory.o" "$OBJ/memory_xdk.o" "$OBJ/hsd_class_xdk.o" \
        ...
        "$OBJ/random.o" "$OBJ/test_hsdmath.o" -lm -o "$OUT"
    "$OUT"
    exit 0
fi
...
cl %CC_NEW% /TC /I%BL% /FI%ROOT%\\src\\xdk\\hsdmath_xdk_compat.h /c %ROOT%\\tests\\host\\test_hsdmath.c /Fo%OBJ%\\test_hsdmath.obj || exit /b 1

link /nologo /OUT:%OBJ%\\test_hsdmath.exe %OBJ%\\memory.obj %OBJ%\\memory_xdk.obj %OBJ%\\hsd_class_xdk.obj %OBJ%\\hash.obj %OBJ%\\debug.obj %OBJ%\\class.obj %OBJ%\\object.obj %OBJ%\\objalloc.obj %OBJ%\\id.obj %OBJ%\\hsdmath_xdk.obj %OBJ%\\mtx.obj %OBJ%\\quatlib.obj %OBJ%\\spline.obj %OBJ%\\fobj.obj %OBJ%\\random.obj %OBJ%\\test_hsdmath.obj || exit /b 1
%OBJ%\\test_hsdmath.exe
exit /b %errorlevel%
BATCH
